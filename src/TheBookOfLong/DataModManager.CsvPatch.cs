...
internal static partial class DataModManager
{
    ...
    private static bool TryMergePlotDataPatch(
        List<List<string>> baseRows,
        ...
        out string? warning)
    {
        ...
        mergedContent = SerializeCsv(mergedRows);
        return true;
    }
    ...
    private static bool UsesPlotDataBlockMerge(string sourcePath)
    {
        return string.Equals(
            ...
            BuildCanonicalSourcePath("GameData/PlotData.csv"),
        ...
        out string? warning)
    {
        ...
        for (int i = 1; i < rows.Count; i += 1)
        {
            if (!TryNormalizeRow(rows[i], columnCount, out List<string>? normalizedRow))
            {
        ...
        out string? warning)
    {
        ...
        for (int i = 0; i < rows.Count; i += 1)
        {
            ...
            string key = GetCell(row, keyColumnIndex);
            if (!string.IsNullOrWhiteSpace(key))
            {
    ...
    private static bool PlotDataBlocksEqual(PlotDataBlock left, PlotDataBlock right)
    {
        ...
        {
            if (!RowsEqual(left.Rows[i], right.Rows[i]))
            {
                return false;
            }
        }
